...
def sampling_process(common_kwargs, worker_kwargs):
    ...
    envs = [c.EnvCls(**c.env_kwargs[env_ind]) for env_ind in range(w.n_envs)]
    ...
    # TODO: distribute assigned eval_env_kwargs
    if c.get("eval_n_envs", 0) > 0:
        eval_envs = [c.EnvCls(**c.eval_env_kwargs[env_ind]) for env_ind in range(c.eval_n_envs)]
